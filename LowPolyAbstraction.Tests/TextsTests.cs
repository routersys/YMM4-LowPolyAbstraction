...
public sealed class TextsTests
{
    ...
    {
        var neutral = Texts.ResourceManager.GetString(nameof(Texts.LowPolyAbstraction), CultureInfo.InvariantCulture);

        Assert.Equal("ローポリ化", neutral);
        Assert.Equal(neutral, Texts.ResourceManager.GetString(nameof(Texts.LowPolyAbstraction), CultureInfo.GetCultureInfo("ja-JP")));
    }
