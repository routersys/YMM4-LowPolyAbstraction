...
internal static class LowPolyAbstractionUpdateNotifier
{
    private const string Owner = "routersys";
    private const string Repository = "YMM4-LowPolyAbstraction";
    ...
    private static async Task RunAsync()
    {
        try
        {
            ...
            {
                CurrentVersion = PluginVersion.FromAssemblyInformationalVersion(typeof(LowPolyAbstractionUpdateNotifier)),
    ...
    private static Task ShowNotificationAsync(UpdateNotification notification, CancellationToken cancellationToken)
    {
        ...
        return application.Dispatcher.InvokeAsync(() =>
            MessageBox.Show(notification.Message, Texts.LowPolyAbstraction, MessageBoxButton.OK, MessageBoxImage.Information)).Task;
    }
}
